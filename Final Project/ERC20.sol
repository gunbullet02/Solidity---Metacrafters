...
pragma solidity ^0.8.17; // solidity compiler version that will be used
...
contract is based upon IERC20 as you can see in the imports and it is an oop method known as inheritance on which you can say this is like a child class of IERC20

    uint public totalSupply; 

//declaring a public variable for token


    mapping(address => uint) public balanceOf; 

// mapping is like hash table that stores key-value pairs - series of addresses stored that is associated with an uint the name is in the last part which is balanceOf

    mapping(address => mapping(address => uint)) public allowance; 

// giving permission to use your tokens also getting their address

    string public name = "Solidity by Example"; 

//Defining the name of the token


    string public symbol = "SOLBYEX"; 

//Defining the symbol token


    uint8 public decimals = 18; 

// assigning the decimal token

// transfer function that takes the address of the recipient and amount 


    function transfer(address recipient, uint amount) external returns (bool) {
        balanceOf[msg.sender] -= amount; 

// the sender sends in nature it subtracts from the sender depending on the amount and is taken on the balance of the sender


        balanceOf[recipient] += amount; 

// the recipient receives the amount sent by the sender which is added to their balance

        emit Transfer(msg.sender, recipient, amount); 

// if the transaction is successful it returns true on both parties on the code it uses global variables like msg.sender which are special variables because it provides information about the blockchain and transactions 
...
// does the opposite of mint which is removing or burning it first gets the amount that it wants to remove of course this gets subtracted to the balance of the user then reevaluating the total supply then after that confirming the transaction.
